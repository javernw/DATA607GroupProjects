...
mydb <-  dbConnect(MySQL(), user = db_user, password = db_password,
                 dbname = db_name, host = db_host, port = db_port)
s <- paste0("SELECT * FROM USChronicDiseaseIndicators;")
rs <- dbSendQuery(mydb, s)
disease <-  fetch(rs, n = -1)
on.exit(dbDisconnect(mydb))
...
```{r}
data.table(head(disease, 50))
```

```{r}
data.table(tail(disease, 50))
```
...
tblTobacco <- subset(disease_no_miss, Topic == "Tobacco" & (!LocationAbbr == "US"))
...
Tobacco_df <- Tobacco_df %>% select("YearEnd", "LocationAbbr", "Question", "DataValue", "Stratification1", "GeoLocation") %>% separate(GeoLocation, c("Latitude", "Longitude"), sep = ",")
