...
For this project, we will only focus on the boroughs and the neighborhoods
```{r}

air_quality <- read.csv(file.choose(), header = T) #Air_Quality.csv
air_q <- air_quality
air_q <- air_q[complete.cases(air_q), ]
...
Collect Zipcode and Matching Neighborhood with UFH42 Code
```{r}
...
Collect Neighborhood and Matching UFH42 Code from pdf file on web
```{r}
...
ufh42_names <- gsub("NYC UHF 42 Neighborhoods", "", text2)
ufh42_names <- gsub("UHF Neighborhood Name", "", ufh42_names)
ufh42_names <- gsub("UHF Code", "", ufh42_names)
ufh42_names <- gsub("Neighborhood Definition:.+", "", ufh42_names)
ufh42_names <- gsub("\\d{3}", "", ufh42_names)
ufh42_names <- unlist(str_extract_all(ufh42_names, "(\\s[A-Za-z]+.\\-?.[A-Za-z]+.\\-?.[A-Za-z]+.[A-Za-z]+.\\-?.[A-Za-z]+| Jamaica| Rockaway| Sunset Park)"))
ufh42_names <- gsub("\\\\r", "", ufh42_names)

neighborhood <- data.frame(Code = ufh42_codes, Location = ufh42_names, stringsAsFactors = F)

#write.csv(neighborhood, file = "Neighborhood.csv", row.names = F)

```

UFH42 Codes and Rent Prices
...
rent <- data.frame(UFH42Code = zip_table$NCode, avg_Rentprice = Rent_prices, stringsAsFactors = F)

#write.csv(rent, file = "Rent.csv", row.names = F)
...
air_rent <- inner_join(air_q, rent, by = c("geo_entity_id" = "UFH42Code"))
